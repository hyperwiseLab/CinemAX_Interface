...
CREATE TABLE IF NOT EXISTS `TBL_USER` (
                            ...
                            `STUDENT_NUM`	VARCHAR(100)	NOT NULL	COMMENT '학번',
                            `STATUS`	TINYINT	NULL	COMMENT '상태',
                            `TERMS_AGREED`	TINYINT(1)	NULL	COMMENT '이용약관 동의',
                            `PRIVACY_AGREED`	TINYINT(1)	NULL	COMMENT '개인정보처리방침 동의',
                            `MARKETING_AGREED`	TINYINT(1)	NULL	COMMENT '마케팅 정보 수신 동의',
                            `MUST_CHANGE_PASSWORD` TINYINT(1) NULL DEFAULT 0 COMMENT '다음 로그인 시 비밀번호 변경 필요',
...
-- ----------------------------
CREATE TABLE IF NOT EXISTS `TBL_CURRICULUM_WEEK` (
...
CREATE TABLE `TBL_CYCLE` (
                             ...
                             `CUR_ID`	BIGINT	NOT NULL	COMMENT '커리큘럼 식별자',
                             `WEEK_NO`	INT(10)	NOT NULL	COMMENT '주차 번호',
                             `SYNTAX_DETAIL_ID`	BIGINT	NULL	COMMENT '상세 문법 식별코드',
                             `SYNTAX_ID`	BIGINT	NULL	COMMENT '문법 식별코드',
                             `CYCLE_TITLE`	VARCHAR(500)	NULL,
                             `FILE_NM`	VARCHAR(200)	NULL	COMMENT '컴파일 파일 명',
                             PRIMARY KEY (`CYCLE_ID`, `CUR_ID`, `WEEK_NO`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `TBL_TASK` (
                            ...
                            `CUR_ID`	BIGINT	NOT NULL	COMMENT '커리큘럼 식별자',
                            `WEEK_NO`	INT(10)	NOT NULL	COMMENT '주차 번호',
                            `TASK_TITLE`	VARCHAR(255)	NULL	COMMENT 'Task 제목',
                            ...
                            `CREATE_DT`	DATETIME	NOT NULL,
                            `UPDATE_DT`	DATETIME	NOT NULL,
                            PRIMARY KEY (`TASK_ID`, `CYCLE_ID`, `CUR_ID`, `WEEK_NO`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `TBL_CLASS` (
                             ...
                             FOREIGN KEY (`CUR_ID`) REFERENCES `TBL_CURRICULUM` (`CUR_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `TBL_LECTURE` (
                               ...
                               `TASK_ID`	BIGINT	NOT NULL	COMMENT '과제/문항 식별자',
                               `CYCLE_ID`	BIGINT	NOT NULL	COMMENT '사이클 식별자',
...
CREATE TABLE `TBL_QUESTION` (
                                ...
                                `CLASS_ID`	BIGINT	NOT NULL	COMMENT '수업 식별자',
                                `WEEKLY_SESSION_ID`	BIGINT	NOT NULL	COMMENT '주차 별 수업 인스턴스 식별 번호',
                                `WEEK_NO`	INT	NULL	COMMENT '주차 번호',
                                `INVITE_ID`	BIGINT	NOT NULL	COMMENT '초대코드 식별자',
...
CREATE TABLE IF NOT EXISTS `TBL_EMAIL_VERIFICATION` (
                                          ...
                                          `USER_ID`	BIGINT	NULL	COMMENT '사용자 일련번호 (회원가입 전에는 NULL)',
                                          `EMAIL`	VARCHAR(200)	NOT NULL	COMMENT '인증할 이메일 주소',
                                          `CODE`	CHAR(6)	NOT NULL	COMMENT '인증번호',
                                          `EXPIRES_AT`	DATETIME	NOT NULL	COMMENT '인증번호 만료 시각',
                                          `ATTEMPT_COUNT`	TINYINT	NOT NULL	COMMENT '인증 시도 횟수(5번 제한)',
                                          `VERIFIED_AT`	DATETIME	NULL	COMMENT '인증 성공 시각',
                                          `CREATE_DT`	DATETIME	NOT NULL	COMMENT '생성 일시',
                                          PRIMARY KEY (`VARIFICATION_ID`),
                                          INDEX `IDX_EMAIL_CREATE_DT` (`EMAIL`, `CREATE_DT`),
                                          CONSTRAINT `FK_TBL_USER_TO_TBL_EMAIL_VERIFICATION_1` FOREIGN KEY (`USER_ID`) REFERENCES `TBL_USER` (`USER_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 외래키는 CREATE TABLE 내에서 정의되었습니다.
-- 기존 테이블이 있고 구조를 변경해야 하는 경우 아래 마이그레이션 스크립트를 수동으로 실행하세요:
-- 
-- 1. 기존 외래키 제거 (존재하는 경우에만):
--    DELIMITER $$
--    CREATE PROCEDURE DROP_FK_IF_EXISTS()
--    BEGIN
--        DECLARE CONTINUE HANDLER FOR 1091 BEGIN END;
--        ALTER TABLE TBL_EMAIL_VERIFICATION DROP FOREIGN KEY FK_TBL_USER_TO_TBL_EMAIL_VERIFICATION_1;
--    END$$
--    DELIMITER ;
...
--
-- 2. 외래키 재추가:
--    ALTER TABLE TBL_EMAIL_VERIFICATION ADD CONSTRAINT FK_TBL_USER_TO_TBL_EMAIL_VERIFICATION_1
--        FOREIGN KEY (USER_ID) REFERENCES TBL_USER (USER_ID);
...
CREATE TABLE `TBL_NOTIFICATION` (
                                    ...
                                    `TYPE`	VARCHAR(100)	NOT NULL	COMMENT '알림 유형 (NotificationType enum)',
...
ALTER TABLE `TBL_HINT` ADD CONSTRAINT `FK_TBL_TASK_TO_TBL_HINT_COMPOSITE` FOREIGN KEY (
                                                                                       `TASK_ID`, `CYCLE_ID`
    )
    REFERENCES `TBL_TASK` (
                           `TASK_ID`, `CYCLE_ID`
...
ALTER TABLE `TBL_TASK` ADD CONSTRAINT `FK_TBL_CYCLE_TO_TBL_TASK_COMPOSITE` FOREIGN KEY (
                                                                                `CYCLE_ID`, `CUR_ID`, `WEEK_NO`
    )
...
ALTER TABLE `TBL_CYCLE` ADD CONSTRAINT `FK_TBL_CURRICULUM_WEEK_TO_TBL_CYCLE_COMPOSITE` FOREIGN KEY (
                                                                                            `CUR_ID`, `WEEK_NO`
    )
...
-- Syntax 외래키는 제거됨 (SYNTAX_ID, SYNTAX_DETAIL_ID가 NULL 허용이므로)
...
-- 외래키는 CREATE TABLE에서 이미 정의되었으므로 주석 처리
...
ALTER TABLE `TBL_ASSIGN` ADD CONSTRAINT `FK_TBL_TASK_TO_TBL_ASSIGN_COMPOSITE` FOREIGN KEY (
                                                                                           `TASK_ID`, `CYCLE_ID`
    )
    REFERENCES `TBL_TASK` (
                           `TASK_ID`, `CYCLE_ID`
...
ALTER TABLE `TBL_FEEDBACK` ADD CONSTRAINT `FK_TBL_TASK_TO_TBL_FEEDBACK_COMPOSITE` FOREIGN KEY (
                                                                                               `TASK_ID`, `CYCLE_ID`
    )
    REFERENCES `TBL_TASK` (
                           `TASK_ID`, `CYCLE_ID`
...
ALTER TABLE `TBL_LECTURE` ADD CONSTRAINT `FK_TBL_TASK_TO_TBL_LECTURE_COMPOSITE` FOREIGN KEY (
                                                                                             `TASK_ID`, `CYCLE_ID`, `CUR_ID`, `WEEK_NO`
    )
    REFERENCES `TBL_TASK` (
                           `TASK_ID`, `CYCLE_ID`, `CUR_ID`, `WEEK_NO`
        );
...
ALTER TABLE `TBL_BRIEF` ADD CONSTRAINT `FK_TBL_TASK_TO_TBL_BRIEF_COMPOSITE` FOREIGN KEY (
                                                                                         `TASK_ID`, `CYCLE_ID`
    )
    REFERENCES `TBL_TASK` (
                           `TASK_ID`, `CYCLE_ID`
...
-- ----------------------------
-- Table structure for TBL_PASSWORD_RESET_REQUEST
-- ----------------------------
CREATE TABLE IF NOT EXISTS `TBL_PASSWORD_RESET_REQUEST` (
    `REQUEST_ID` BIGINT NOT NULL AUTO_INCREMENT COMMENT '요청 ID',
    `EMAIL` VARCHAR(200) NOT NULL COMMENT '이메일',
    `VERIFICATION_CODE` VARCHAR(6) NOT NULL COMMENT '인증 코드',
    `EXPIRES_AT` DATETIME NOT NULL COMMENT '만료 시간',
    `USED_AT` DATETIME NULL COMMENT '사용 시간',
    `ATTEMPT_COUNT` INT(10) NOT NULL DEFAULT 0 COMMENT '시도 횟수',
    `RESET_METHOD` VARCHAR(20) NOT NULL COMMENT '재설정 방식',
    `STATUS` VARCHAR(20) NOT NULL DEFAULT 'PENDING' COMMENT '상태',
    `USER_ID` BIGINT NULL COMMENT '사용자 ID',
    `CREATE_DT` DATETIME NOT NULL COMMENT '생성 시간',
    `UPDATE_DT` DATETIME NOT NULL COMMENT '수정 시간',
    PRIMARY KEY (`REQUEST_ID`),
    INDEX `IDX_PASSWORD_RESET_EMAIL` (`EMAIL`),
    INDEX `IDX_PASSWORD_RESET_CODE` (`VERIFICATION_CODE`),
    INDEX `IDX_PASSWORD_RESET_EXPIRES` (`EXPIRES_AT`),
    INDEX `IDX_PASSWORD_RESET_STATUS` (`STATUS`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Foreign key for TBL_PASSWORD_RESET_REQUEST
ALTER TABLE `TBL_PASSWORD_RESET_REQUEST` ADD CONSTRAINT `FK_TBL_USER_TO_TBL_PASSWORD_RESET_REQUEST_1` FOREIGN KEY (
    `USER_ID`
) REFERENCES `TBL_USER` (
    `USER_ID`
);
...
ALTER TABLE TBL_PROGRESS ADD COLUMN LAST_ACTIVITY_TIME DATETIME,
    ADD COLUMN TEST_FAIL_COUNT INT DEFAULT 0, ADD COLUMN ACTIVITY_STATUS VARCHAR(20);
-- ----------------------------
-- CBT (자격증 모의 CBT) 테이블
-- 반(CLASS) = 자격증 1개, 과목별 랜덤 출제 / 과락·합불 판정 / 회차별 기록
-- class/user 는 단순 참조값 (FK 제약 없음 - 반/유저 삭제 차단 방지)
-- ----------------------------
CREATE TABLE IF NOT EXISTS `TBL_CBT_CONFIG` (
    `CONFIG_ID` BIGINT NOT NULL AUTO_INCREMENT COMMENT '설정 ID',
    `CLASS_ID` BIGINT NOT NULL COMMENT '반 ID',
    `PASS_SCORE` DOUBLE NOT NULL DEFAULT 60 COMMENT '최종 합격 점수(100점 기준)',
    `CREATE_DT` DATETIME NOT NULL COMMENT '생성 시간',
    `UPDATE_DT` DATETIME NOT NULL COMMENT '수정 시간',
    PRIMARY KEY (`CONFIG_ID`),
    UNIQUE KEY `UK_CBT_CONFIG_CLASS` (`CLASS_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS `TBL_CBT_SUBJECT` (
    `SUBJECT_ID` BIGINT NOT NULL AUTO_INCREMENT COMMENT '과목 ID',
    `CLASS_ID` BIGINT NOT NULL COMMENT '반 ID',
    `SUBJECT_NM` VARCHAR(100) NOT NULL COMMENT '과목명',
    `ORDER_NO` INT NOT NULL COMMENT '표시 순서',
    `POINT_PER_QUESTION` DOUBLE NOT NULL COMMENT '문제당 점수',
    `QUESTION_COUNT` INT NOT NULL COMMENT '회차당 출제 문항 수',
    `CUT_SCORE` DOUBLE NOT NULL COMMENT '과락 기준점(배점 내 절대점수)',
    `CUT_YN` TINYINT(1) NOT NULL DEFAULT 1 COMMENT '과락 적용 여부',
    `USE_YN` TINYINT(1) NOT NULL DEFAULT 1 COMMENT '사용 여부',
    `CREATE_DT` DATETIME NOT NULL COMMENT '생성 시간',
    `UPDATE_DT` DATETIME NOT NULL COMMENT '수정 시간',
    PRIMARY KEY (`SUBJECT_ID`),
    INDEX `IDX_CBT_SUBJECT_CLASS` (`CLASS_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS `TBL_CBT_QUESTION` (
    `QUESTION_ID` BIGINT NOT NULL AUTO_INCREMENT COMMENT '문제 ID',
    `CLASS_ID` BIGINT NOT NULL COMMENT '반 ID',
    `SUBJECT_ID` BIGINT NOT NULL COMMENT '과목 ID',
    `CONTENT` TEXT NOT NULL COMMENT '지문',
    `EXPLANATION` VARCHAR(1000) NULL COMMENT '해설',
    `USE_YN` TINYINT(1) NOT NULL DEFAULT 1 COMMENT '사용 여부',
    `CREATED_BY` BIGINT NULL COMMENT '등록 관리자 ID',
    `CREATE_DT` DATETIME NOT NULL COMMENT '생성 시간',
    `UPDATE_DT` DATETIME NOT NULL COMMENT '수정 시간',
    PRIMARY KEY (`QUESTION_ID`),
    INDEX `IDX_CBT_QUESTION_CLASS_SUBJECT` (`CLASS_ID`, `SUBJECT_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS `TBL_CBT_QUESTION_OPTION` (
    `OPTION_ID` BIGINT NOT NULL AUTO_INCREMENT COMMENT '보기 ID',
    `QUESTION_ID` BIGINT NOT NULL COMMENT '문제 ID',
    `ORDER_NO` INT NOT NULL COMMENT '보기 순서(1~4)',
    `CONTENT` VARCHAR(1000) NOT NULL COMMENT '보기 내용',
    `CORRECT_YN` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '정답 여부',
    `CREATE_DT` DATETIME NOT NULL COMMENT '생성 시간',
    `UPDATE_DT` DATETIME NOT NULL COMMENT '수정 시간',
    PRIMARY KEY (`OPTION_ID`),
    INDEX `IDX_CBT_OPTION_QUESTION` (`QUESTION_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS `TBL_CBT_ATTEMPT` (
    `ATTEMPT_ID` BIGINT NOT NULL AUTO_INCREMENT COMMENT '응시 ID',
    `CLASS_ID` BIGINT NOT NULL COMMENT '반 ID',
    `USER_ID` BIGINT NOT NULL COMMENT '학생 ID',
    `ROUND_NO` INT NOT NULL COMMENT '학생별 회차',
    `TOTAL_SCORE` DOUBLE NOT NULL COMMENT '총점(100점 만점)',
    `PASS_YN` TINYINT(1) NOT NULL COMMENT '합격 여부',
    `SUBJECT_SCORES_JSON` JSON NULL COMMENT '과목별 점수/과락 스냅샷',
    `ANSWERS_JSON` JSON NULL COMMENT '문항별 답안 스냅샷',
    `SUBMITTED_AT` DATETIME NOT NULL COMMENT '제출 시간',
    `CREATE_DT` DATETIME NOT NULL COMMENT '생성 시간',
    `UPDATE_DT` DATETIME NOT NULL COMMENT '수정 시간',
    PRIMARY KEY (`ATTEMPT_ID`),
    INDEX `IDX_CBT_ATTEMPT_CLASS_USER` (`CLASS_ID`, `USER_ID`),
    INDEX `IDX_CBT_ATTEMPT_CLASS` (`CLASS_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
